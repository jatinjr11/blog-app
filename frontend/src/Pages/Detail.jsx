...
const Detail = () => {
  ...
  const handleComment = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:4001/api/blogs/comment/${id}`,
        { text: newComment },
        { withCredentials: true }
      );
  ...
  const handleReply = async (commentId) => {
    ...
    try {
      await axios.post(
        `http://localhost:4001/api/blogs/reply/${id}/${commentId}`,
        { text },
        { withCredentials: true }
      );
  ...
  const handleDeleteReply = async (commentId, replyId) => {
    try {
      await axios.delete(
        `http://localhost:4001/api/blogs/reply/delete/${id}/${commentId}/${replyId}`,
        { withCredentials: true }
      );
  ...
  const handleCommentLike = async (commentId) => {
    try {
      const res = await axios.post(
        `http://localhost:4001/api/blogs/comments/like/${commentId}`,
        {},
        { withCredentials: true }
      );
      toast.success(res.data.message);
  ...
  const handleEditComment = async (commentId) => {
    try {
      await axios.put(
        `http://localhost:4001/api/blogs/comment/edit/${id}/${commentId}`,
  ...
  const handleDeleteComment = async (blogId, commentId) => {
    try {
      await axios.delete(
        `http://localhost:4001/api/blogs/comment/delete/${blogId}/${commentId}`,
        { withCredentials: true }
      );
      toast.success("Comment deleted");

      const { data } = await axios.get(
        `http://localhost:4001/api/blogs/single-blog/${blogId}`,
        { withCredentials: true }
      );
      setComments(data.comments);
    } catch (err) {
